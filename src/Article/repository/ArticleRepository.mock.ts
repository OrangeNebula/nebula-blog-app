...
첫번째로 경험에 대한 부분이다. 경험은 책이나 다른 매체로 간접적으로 접할 수 있는 부분도 있지만, 가장 좋은 경험은 몸으로, 머리로 직접 부딪혀가며 접할 수 있는 경험이 가장 값지다고 생각한다. 그 이유는 간접적으로 접한 경험의 경우는 진정으로 자기것으로 만들기 어려울 때가 많고, 내 머리속에 각인되기 어려워 휘발될 수도 있기 때문이다. 나 역시도 많은 대가를 지불하면서 몸으로 배운 경험은 아직까지 잘 잊지 않는 반면, 책으로 접한 경험은 머리로는 이해해도 그것을 학습해서 내 행동의 변화로 이어지기가 어려웠다. 가장 먼저 경험의 영역은 이런 저런 시도를 해보면서 경험적으로 습득하는것이 좋은것 같다. 단순히 책이나, 다른 사람의 가르침으로부터 전달되는 정보로 경험치를 쌓는데에는 충분하지 않을 수 있다. 혼자 이런저런 토이 프로젝트를 진행해보거나, 마음 맞는 사람들과 사이드 프로젝트나 스터디를 해보면서 몸으로 익혀나가는게 도움이 되는것 같다. 물론 현업에서 요구하는 경험치를 채우는데는 한계가 있을 수 있지만, 실무에서는 리스크가 따를 수 있는 새로운 시도나 실험을 안전하게 할 수 있다는 장점도 있다. 물론 조직내에서 안전하게 경험하고, 성장할 수 있는 환경이 조성되어 있다면야 더할나위 없겠지만, 이런 환경을 조성하기엔 많은 노력과 시간이 든다. 이에 대해서는 다음에 더 정리해볼려고 한다.
