...
두번째는 어떻게 코드 품질을 높일 수 있을까 인데, 이에 대해 답하기 위해선 어떤 코드가 좋은 코드인가에 대한 질문으로 연결된다. 이것 역시 어려운 주제이기에 다음에 더 깊게 생각을 정리해보면 좋겠다. 여기서는 개발자로써의 성장 측면에서만 간단하게 정리를 해보겠다. 수년간 "좋은 코드는 어떤 코드인가?" 에 대해서 많은 개발자들의 생각을 여쭤보고 다녔다. 일반적으로 생각하는 좋은 코드부터, 생각하지도 못한 창의적인 접근까지 다양한 정의를 들은것 같다. 그나마 지금까지 접한 정의들 중에 가장 마음이 가는 정의는 마이클 페더스의 레거시 코드 활용 전략이라는 책에서 얻은 "테스트가 용이한 코드" 였다. 코드는 기대한대로 동작해야 의미가 있다. 잘 읽히는 코드, 확장성 있는 코드, 관심사 분리가 잘 된 코드 등 좋은 코드의 다양한 조건들이 있겠지만, 결국 이 모든것은 요구사항대로 잘 동작하는것이 선행되고 난 후에야 이야기 될 수 있다. 그렇다면 어떻게 요구사항대로 잘 동작하는지 확인을 "잘" 할 수 있을까? 바로 테스트 코드가 도움을 줄 수 있다. 그럼 테스트 코드를 잘 작성하려면 어떻게 해야 할까? 테스트가 용이해야 할 것이다. 코드가 테스트에 용이하다면, 테스트 코드를 잘 작성할 수 있을것이고, 테스트 코드를 이용하여 내가 잘 코드를 작성했는지 빠르게 동작여부에 대한 피드백을 받을 수 있다. 또한 버그 픽스에 드는 시간도 줄 일 수 있다(일반적으로 나중에 발견되는 버그일 수록 더 고통스러운 대가를 치르는것 같다). 좋은 코드에 대한 생각은 개발자들마다 다를 수 있다. 핵심은 내가 테스트가 용이한 코드가 좋은 코드라고 생각하는 것처럼, 좋은 코드란 무엇인지? 내가 작성하는 코드가 좋은 코드가 되기 위해선 어떻게 해야하는지? 치밀하게 고민해보는 것, 그것이 바로 핵심이 되겠다.

세번째는 지식 수준이다. 기술에 대한 지식 수준은 정보를 더 많이 가지고 있는 시니어가 유리한건 당연하다. 그러나 시간에 따라서 기술은 변한다(물론 기술마다 차이가 있다). 그 말은 새로운 기술일 수록 시니어와 주니어 둘 다 새로운 출발점을 지나야 하기에, 해당 기술에 대해서는 지식 수준이 큰 차이가 나지 않을 수도 있다. 또한 같은 지식을 접해도 사람마다 이해하는 시간, 내것으로 만드는 시간, 활용으로 넘어가는 시간 다르다. 이 말은 현재의 지식의 수준도 중요하지만, 지식 수준을 높이는 러닝 어질리티가 역시 중요하다는 이야기가 될 것 같다. 과연 시니어가 무조건 러닝 어질리티가 높다고 볼 수 있을까? 그것은 또 다른 주제일것 같다. 확실한건, 지식은 시간에 따라 변하기 때문에 시니어가 모든 정보를 알 고 있기는 힘들고, 그런 무리한 기대를 해서도 안되며, 반대로 주니어는 무조건 모른다는 억측을 해서도 안된다는 것이다. 계속해서 배우고 내 것으로 만들기 위해 끊임없이 노력해야 한다. 관심있는 기술 블로그를 주기적으로 확인하고, 책을 읽고, 내 기술 스택의 업데이트 사항들을 f/up 하고... 개발자는 평생 공부해야 한다는 이야기가 허투로 말하는게 아닌 이유가 여기에 있다고 본다.

성장에 대한 세가지 꼭지를 살펴보았다. 다른 직업군에서도 성장에 대해서 치열하게 고민할거라 생각한다. 그런데 개발자는 평생 공부해야 한다는 이야기에서도 느껴지지만, 참 공부해야 할 것도 많고, 갖춰야 할 것도 많은 것 같다. 그래서 이런 개발자들에게 하고 싶었던 내용을 다음글에 정리해보고자 한다. 
